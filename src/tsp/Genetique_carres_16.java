...
/**Dans cette classe genetique_carré_16 on découpe géographique en carrés l'espace où se situent les villes. Toutes les villes 
 * appartiennent donc à un carré de 1 à 16.
 ...
 * Deux configuration de l'espace sont testé (voir la classe Genetique_carres_4) et il apparait que le découpage en trop petits carré (16) n'est pas optimal.
 ...
 * 
 * 
 * Les fonctions spécifiques au découpage en 16 :
 * Numero_carre_16 (int index_ville, Instance instance)
 * int[] tableau_ville_16(Instance instance)
 * ArrayList<Solution> CreerPopulation_Carre_16
 * int get_point_de_croisement_16 (Instance instance)
 * Solution Mutation_16 (Solution individu)
 * ArrayList<Solution> Completer_Pop_16(ArrayList<Solution> population1, ArrayList<Solution> population2)
 * ArrayList<Solution> Modifier_doublon_16(ArrayList<Solution> population)
 * 
...
public class Genetique_carres_16 {
	private static Instance m_instance;
	private static int nbIndividu;

	public Genetique_carres_16 (Instance m_instance, int nbIndividu) {
	...
	/**
	 * On découpe géographiquement la carte des villes en 16 carrés de tailles identiques
	...
	public static int Numero_carre_16 (int index_ville, Instance instance) throws Exception {
		m_instance = instance;
		double coordonne_x=m_instance.getX(index_ville);
		double coordonne_y=m_instance.getY(index_ville);
		int res;
		if(coordonne_x<= m_instance.getMaxX()/4) {
			if(coordonne_y <= m_instance.getMaxY()/4) {
				res = 1;
			}
			else {
				if(coordonne_y<= m_instance.getMaxY()/2) {
					...
					if(coordonne_y<= m_instance.getMaxY()/(3/4)) {
						res = 9;
					}
					else {
						res=13;
				...
				if(coordonne_x<=m_instance.getMaxX()/2) {
					...
					if(coordonne_y<= m_instance.getMaxY()/2) {
						...
						if(coordonne_x<=m_instance.getMaxX()/(3/4)) {
	...
	/**Pour un découpage en 16 carrés
	 ...
	 * @throws Exception
	 */
	public static int[] tableau_ville_16(Instance m_instance) throws Exception{
		int[] tab = new int[m_instance.getNbCities()];
		for(int i=0; i<m_instance.getNbCities(); i++) {
			tab[i]=Numero_carre_16(i, m_instance);
			}
		return tab;
	}
	
	/**
	 * On crée une population composée de nbIndividu, chaque individu doit respecter le découpage en carrés géographiques de 16
	 * On met d'abord toutes les villes d'un même carré ( ici le 1) puis toutes les villes du carré 2 
	 * en respectant l'ordre 1,2,3,4,8,7,6,10,11,12,16,15,14,13,9,5
	 * @param instance de type Instance
	...
	public static ArrayList<Solution> CreerPopulation_Carre_16(Instance instance, int nbIndividu, int[] tableau_ville ) throws Exception {
		...
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==8) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==7) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==6) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==10) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==11) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==12) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==16) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==15) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==14) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==13) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==9) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		for(int i=1; i<m_instance.getNbCities(); i++) {
			if(tableau_ville[i]==5) {
				m_solution.setCityPosition(i,position);     	 	  		  		    		 	
				position++;
			}
		}
		...
		for(int j=0; j<nbIndividu; j++) { 
			...
			for(int m=0; m<300; m++) {
				...
				if(pos_a_echanger1!=0 && pos_a_echanger2!=0 && Numero_carre_16(index_city1, m_instance)==Numero_carre_16(index_city2, m_instance)) {
	 ...
	 * d'un carré donc on compte le nombre de villes qu'il y a dans les 4 premiers carrés si découpage géographique en 16
	 * 
	 * @param instance
	 * @return retourne un point de séparation 
	 * @throws Exception
	 */
	public static int get_point_de_croisement_16 (Instance m_instance) throws Exception{
		int [] tableau = tableau_ville_16(m_instance);
		int compteur = 0;
		for(int i=0; i<m_instance.getNbCities(); i++) {
			if(tableau[i]==1 || tableau[i]==2 || tableau[i]==3 || tableau[i]==4) {
	...
	public static Solution Mutation_16 (Solution individu) throws Exception {
		...
		for(int i=0; i<nb_mutation; i++) {
			...
			if(Numero_carre_16(index_city1, m_instance)==Numero_carre_16(index_city2, m_instance)) {
	...
	public static ArrayList<Solution> MutationPop_16 (ArrayList<Solution> population ) throws Exception {
	...
	public static ArrayList<Solution> Completer_Pop_16(ArrayList<Solution> population1, ArrayList<Solution> population2) throws Exception{
		...
		if(nbIndividu_manquant>0) {
			for(int j=0; j<nbIndividu_manquant; j++) {
				Solution indi = Mutation_16(population2.get(j));
				pop.add(indi);
			}
		}
	...
	public static ArrayList<Solution> Modifier_doublon_16(ArrayList<Solution> population) throws Exception{
		...
		for(Solution s : population) {
			...
			if(distances.contains(distance_totale)) {
				Solution indiv_mutation=Mutation_16(s);
